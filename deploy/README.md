...
Use a small Ubuntu 24.04 VPS (x86-64 or ARM64) with Docker Engine and the Compose plugin. This deployment is restricted to test client IPs; it is not public customer onboarding.

1. Point `connect.routibot.com` directly to the VPS with a DNS A record (and AAAA only if IPv6 works). Keep proxy/CDN mode off for this pilot so the IP allowlist sees the real clients.
...
Replace the IP placeholders with the actual public addresses of the test clients. Omit IPv6 if unused. Caddy requires this list and responds 403 to everyone else. Certificate issuance is managed by Caddy. Certificates persist in Docker volumes; access logs are not enabled.
...
To stop: `docker compose down` (keep volumes for certificates). To revoke: remove the pair from relay.json and recreate the relay container. Persistent endpoint-side revocation and automated certificates for paired devices are still app-integration work.
