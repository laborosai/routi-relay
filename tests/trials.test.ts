...
test('trial starts with the first phone connection, persists, and closes only expired sessions', { timeout: 10_000 }, async t => {
  ...
  let saved: Trial[] = []
  let relay = createRelay([pilot], { saveTrials: next => { saved = structuredClone(next) }, heartbeatMs: 20 })
  t.after(() => relay.close())
  ...
  await relay.close()
  relay = createRelay([pilot], { trials: saved, saveTrials: next => { saved = structuredClone(next) }, heartbeatMs: 20 })
  base = await listen()
  await control(mac)
  const resumed = await join(mac, phone, 'desktop')
  ...
  assert.equal((await enroll()).status, 200)
  assert.equal(saved[0]!.expiresAt, deadline)
  const rejected = socket(phone, '/v1/sessions/again')
